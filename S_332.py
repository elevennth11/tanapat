...
try:
    ...
    while True:
        ...
        data, server_address = server_socket.recv(4096)
        command = data.decode().lower()
        if command == "passwordformclient":
            response = correct_password
            server_socket.send(response.encode(), server_address)
        data1, server_address1 = server_socket.recv(4096)
        ...
        if data_arr[0] == "connect to server":
            ...
            server_socket.send(response.encode(), server_address1)
        while 1:
            data2, server_address2 = server_socket.recv(4096)
            if data2.decode().lower() == "temp":
                ...
                server_socket.send(data_pack, server_address2)
